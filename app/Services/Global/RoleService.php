...
class RoleService
{
    ...
    public function getModels(...$exceptions): Collection
    {
        ...
        return $generalModels->merge($moduleModels)->merge($additionalModels)->filter(fn($model) => !in_array($model, $exceptions));
    }
    ...
    private function getModuleModels(): Collection
    {
        ...
        $moduleDirectories = collect(File::directories($modulesPath))->filter(fn($directory) => !Str::startsWith(basename($directory), '.'));
    ...
    {
        return collect($this->prepareOperations($modelName))->map(fn($operation) => $this->findOrCreatePermission($modelName, $operation));
    }
    ...
    private function findOrCreatePermission(string $model, string $operation): Permission
    {
        ...
        return Permission::firstOrCreate(
            ...
            [
                'group' => $modelName,
                'display_name' => [
                    'en' => resolveTrans($model, lang: 'en') . ' (' . resolveTrans($operation, lang: 'en') . ')',
                    'ar' => resolveTrans($model, lang: 'ar') . ' (' . resolveTrans($operation, lang: 'ar') . ')',
                ],
            ],
        );
    }
    ...
    {
        return $models->flatMap(fn($model) => $this->createModelPermissions($model))->each(fn($permission) => $role->givePermissionTo($permission));
    }
    ...
    private function prepareOperations(string $modelName): array
    {
        ...
        if ($additionalOperation = collect($additionalOperations)->firstWhere('name', $modelName)) {
            $operations = isset($additionalOperation['basic']) ? array_unique(array_merge($operations, $additionalOperation['operations'])) : $additionalOperation['operations'];
        }
    ...
    private function handleCustomOperation(array $models): array
    {
        return collect($models)
            ->flatMap(function ($operations, $model) {
    ...
    {
        return collect($models)->flatMap(fn($model) => $this->createModelPermissions($model)->pluck('name')->toArray())->unique()->toArray();
    }
}
